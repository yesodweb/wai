...
assertFailure msg = msg `deepseq` liftIO (throwIO (WaiTestFailure location msg))
  where
    location :: Maybe GHC.SrcLoc
    location = case reverse (GHC.getCallStack $ GHC.popCallStack GHC.callStack) of
      (_, loc) : _ -> Just loc
      [] -> Nothing

data WaiTestFailure = WaiTestFailure (Maybe GHC.SrcLoc) String
    deriving (Eq, Typeable)
instance Exception WaiTestFailure

instance Show WaiTestFailure where
    show (WaiTestFailure (Just loc) str) =
        GHC.srcLocFile loc ++ ":" ++ show (GHC.srcLocStartLine loc) ++ ":\n" ++ str
    show (WaiTestFailure Nothing str) =
        str
