...
spec = describe "Connection header" $ do
    it
        "sends Connection: close when response implies close (HTTP/1.0 Keep-Alive, no Content-Length)" $ do
    ...
    describe "HTTP/1.1 Connection: close behavior" $ do
        ...
        it
            "sends Connection: close when request has Connection: close (500 Internal Server Error)" $ do
